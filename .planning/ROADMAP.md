...
- [x] 02-03-PLAN.md — dock_scan_capture rclpy library (5-frame voxel-merge, atomic write) + calibrate_imu_yaw_node extension (R-1, no extra operator step) + 13 pytests (7 capture + 6 wiring) → SUMMARY at `.planning/phases/02-lidar-dock-pose-estimation/02-03-SUMMARY.md` (commits `b0d8df1a`, `78fbbdff`, `7e42e576`)
- [x] 02-04-PLAN.md — dock_scan_match node + KinematicIcpDockMatcher production impl + mtime watcher + TF distance gate + degraded mode + navigation.launch.py wiring + 11 gtests (R-2, R-3) → SUMMARY at `.planning/phases/02-lidar-dock-pose-estimation/02-04-SUMMARY.md` (commits `6409bf34`, `305d6dc9`, `48ca6c20`)
