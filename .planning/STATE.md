...
20 (Phase 1: 11 plans, Phase 2: 9 plans)
...
- **Next step:** Sync feature branch to Pi5 (`pi@10.10.40.101` via Alfa stick — primary; `.68` onboard wlan0 as fallback), build the ROS2 workspace there with `colcon build` against the dev compose stack, then walk `02-08-PI5-CHECKLIST.md` end-to-end (5-of-5 UAT — R-7 / R-9 / R-11 hardware gates). Commit results to `02-08-PI5-RESULTS.md`. Then run `/gsd-verify-phase 2` to flip GAP-02 row in `02-VERIFICATION.md` to `✓ VERIFIED`. Optional intermediate: run regression gtest `test_kinematic_icp_headers_reachable` inside the container (`colcon test --packages-select mowgli_lidar_docking -R test_kinematic_icp_headers_reachable`) — needs a fresh build with `-DBUILD_TESTING=ON`, since the production stage ships with `BUILD_TESTING=OFF`.
