...
- **Sensor fusion:** FusionCore (single UKF, 100Hz, GPS+IMU+wheels+optional Kinematic-ICP twist → odom→base_footprint TF + `/fusion/odom`). Source-built from `ros2/src/fusioncore/`. Lifecycle node auto-configured at launch. Key tuning: `gnss.apply_lever_arm_pre_heading: false` in `localization.yaml` (deliberate — `true` caused ~0.3 m/s phantom base_link motion during pure in-place rotation when yaw was off, see comment in `localization.yaml:171-186` and session 2026-04-23-heightfilter-v1). With `false`, the GPS lever arm only switches on after `heading_validated_` flips — either via an accepted dock-heading update or after 5 m of straight GPS track. `imu.lever_arm_x/y/z` can be auto-resolved from TF (`base_footprint → imu_link`) if left at 0, so the URDF is the single source of truth for sensor geometry. The `hardware_bridge_node` runs a 20 s IMU bias calibration (`imu_cal_samples: 1000`) every time the robot docks, and logs the implied mounting pitch/roll so the operator can promote any >1° offset into `mowgli_robot.yaml` → `imu_pitch/imu_roll`.
